...
onMounted(async () => {
    try {
        const response = await import('../textosbases/bt-capitulo-6-expectativas-de-la-nueva-solucion.json'); // Asegúrate que la ruta sea correcta
        capituloData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Capítulo 6 (Bases Técnicas):', error);
        errorMessage.value = 'No se pudo cargar la información del Capítulo 6 (Bases Técnicas).';
    }
});
</script>
...
<template>
    <div class="capitulo-container">
        ...
        <p v-else class="loading-message">Cargando "Capítulo 6: Expectativas de la Nueva Solución (Bases Técnicas)"...
        </p>
    </div>
</template>
