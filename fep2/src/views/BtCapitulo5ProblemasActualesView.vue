...
onMounted(async () => {
    try {
        const response = await import('../textosbases/bt-capitulo-5-problemas-actuales.json'); // Asegúrate que la ruta sea correcta
        capituloData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Capítulo 5 (Bases Técnicas):', error);
        errorMessage.value = 'No se pudo cargar la información del Capítulo 5 (Bases Técnicas).';
    }
});
</script>
    ...
    <div class="capitulo-container">
        ...
        <div v-if="capituloData && capituloData.secciones_principales" class="content">
            <div v-for="seccion in capituloData.secciones_principales" :key="seccion.numero" class="section-block">
                ...
                <!-- Subsecciones con listas de puntos (para KPIs y Dolores Actuales) -->
                ...
                <!-- Listas de puntos directas (para Productividad) -->
        ...
        <p v-else class="loading-message">Cargando "Capítulo 5: Problemas Actuales (Bases Técnicas)"...</p>
    </div>
</template>
