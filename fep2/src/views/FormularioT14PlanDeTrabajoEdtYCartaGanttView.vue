...
onMounted(async () => {
    try {
        const response = await import('../textosbases/formulario-t14-plan-de-trabajo-edt-y-carta-gantt.json'); // Asegúrate que la ruta sea correcta
        formData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Formulario T-14:', error);
        errorMessage.value = 'No se pudo cargar la información del Formulario T-14.';
    }
});
</script>
...
<template>
    <div class="formulario-container">
        ...
        <p v-else class="loading-message">Cargando "Formulario T-14"...</p>
    </div>
</template>
