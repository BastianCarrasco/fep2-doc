...
onMounted(async () => {
    try {
        const response = await import('../textosbases/formulario-t17-protocolo-de-aceptacion.json'); // Asegúrate que la ruta sea correcta
        formData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Formulario T-17:', error);
        errorMessage.value = 'No se pudo cargar la información del Formulario T-17.';
    }
});
</script>
...
<template>
    <div class="formulario-container">
        ...
        <p v-else class="loading-message">Cargando "Formulario T-17"...</p>
    </div>
</template>
