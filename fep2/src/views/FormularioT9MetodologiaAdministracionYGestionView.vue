...
onMounted(async () => {
    try {
        const response = await import('../textosbases/formulario-t9-metodologia-administracion-y-gestion.json'); // Asegúrate que la ruta sea correcta
        formData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Formulario T-9:', error);
        errorMessage.value = 'No se pudo cargar la información del Formulario T-9.';
    }
});
</script>
...
<template>
    <div class="formulario-container">
        ...
        <p v-else class="loading-message">Cargando "Formulario T-9"...</p>
    </div>
</template>
