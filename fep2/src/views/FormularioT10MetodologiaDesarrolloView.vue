...
onMounted(async () => {
    try {
        const response = await import('../textosbases/formulario-t10-metodologia-desarrollo.json'); // Asegúrate que la ruta sea correcta
        formData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Formulario T-10:', error);
        errorMessage.value = 'No se pudo cargar la información del Formulario T-10.';
    }
});
</script>
...
<template>
    <div class="formulario-container">
        ...
        <p v-else class="loading-message">Cargando "Formulario T-10"...</p>
    </div>
</template>
