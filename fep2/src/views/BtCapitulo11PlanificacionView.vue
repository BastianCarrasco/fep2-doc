...
onMounted(async () => {
    try {
        const response = await import('../textosbases/bt-capitulo-11-planificacion.json'); // Asegúrate que la ruta sea correcta
        capituloData.value = response.default;
    } catch (error) {
        console.error('Error al cargar el Capítulo 11 (Bases Técnicas):', error);
        errorMessage.value = 'No se pudo cargar la información del Capítulo 11 (Bases Técnicas).';
    }
});
</script>
...
<template>
    <div class="capitulo-container">
        ...
        <p v-else class="loading-message">Cargando "Capítulo 11: Planificación (Bases Técnicas)"...</p>
    </div>
</template>
